    ...
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          ...
          { label: 'Cancelled', count: appointments.filter(a => a.status === 'cancelled').length, color: 'bg-red-50 text-red-700' },
        ].map((stat, i) => (
            ...
            {appointments.map(apt => {
              ...
              return (
                <div key={apt._id}
                  ...
                  <img
                    src={doc?.photo || `https://ui-avatars.com/api/?name=${doc?.name}&background=0ea5e9&color=fff`}
                    alt={doc?.name}
...
{apt.otp && (
  <p className="text-xs text-blue-700 font-medium mt-1">
    ...
    <span className="text-gray-400 ml-2 font-normal">
      {new Date() < new Date(apt.otpExpiresAt)
        ? `(Valid until ${new Date(apt.otpExpiresAt).toLocaleTimeString('en-LK', { hour: '2-digit', minute: '2-digit' })})`
        : '(OTP has expired)'}
    </span>
  </p>
)}
